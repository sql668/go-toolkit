...
import (
	...
	gtredis "github.com/sql668/go-toolkit/contrib/redis"
...
type ConsumerFunc func(*gtsm.Message) error
...
type ConsumerOptions struct {
	...
	RedisOptions *gtredis.RedisOptions
}
...
func NewConsumerWithOptions(options *ConsumerOptions) (*Consumer, error) {
	...
	if options.RedisClient != nil {
		...
		r = gtredis.NewRedisClient(options.RedisOptions)
	}

	if err := gtredis.RedisPreflightChecks(r); err != nil {
		return nil, err
	}
	...
	return &Consumer{
		...
		queue:     make(chan *gtsm.Message, options.BufferSize),
		wg:        &sync.WaitGroup{},
		...
		case <-ticker.C:
			for stream := range c.consumers {
				...
				for {
					...
					newID, err := gtredis.IncrementMessageID(res[len(res)-1].ID)
					if err != nil {
						c.Errors <- err
						break
					}
...
func (c *Consumer) process(msg *gtsm.Message) (err error) {
	defer func() {
		if r := recover(); r != nil {
			if e, ok := r.(error); ok {
