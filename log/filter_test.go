...
func TestFilterLevel(_ *testing.T) {
	logger := With(DefaultLogger, "ts", DefaultTimestamp, "caller", DefaultCaller)
	log := NewHelper(NewFilter(NewFilter(logger, FilterLevel(WarnLevel))))
	log.Log(DebugLevel, "msg1", "te1st debug")
...
func TestFilterCaller(_ *testing.T) {
	...
	_ = log.Log(DebugLevel, "msg1", "te1st debug")
	logHelper := NewHelper(NewFilter(logger))
	logHelper.Log(DebugLevel, "msg1", "te1st debug")
}
...
func TestFilterFuncWitchLoggerPrefix(t *testing.T) {
	...
	for _, tt := range tests {
		err := tt.logger.Log(InfoLevel, "msg", "msg", "filtered", "true")
		if err != nil {
...
func TestFilterWithContext(t *testing.T) {
	...
	filter := NewFilter(logger, FilterLevel(ErrorLevel))

	ctx := context.WithValue(context.Background(), ctxKey, ctxValue)

	_ = WithContext(ctx, filter).Log(InfoLevel, "kind", "test")
	...
	_ = WithContext(ctx, filter).Log(ErrorLevel, "kind", "test")
